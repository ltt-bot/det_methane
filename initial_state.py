...
def main():
    ...
    # Run the algorithm
    result = minimize(full_CJ_algorithm, x0=[3600.0, 1_850_000.0], method='Powell', tol=1e-10, options= {'xtol':1e-3, 'maxfev':50000})
    ...
    final_gamma = gas.cp_mass / gas.cv_mass
    print("Specific heat ratio (gamma) for burned gas: {:.2f}".format(final_gamma))
    print("--------------------------------")
    print("Heat release of hydrogen (H2) - oxygen (O2) combustion: {:.2f} MJ/kg".format(qc_final/(10**6)))
    print("--------------------------------")
    mach_plus_CJ, v_hat_CJ, p_hat_CJ = compute_CJ_solution(reac_rho, reac_pressure, final_gamma, qc_final)
    ...
    print(f"P burned for CJ state is {(p_hat_CJ*reac_pressure)/101_325:.2f} atm ({(p_hat_CJ*reac_pressure):.2f} pa)")
